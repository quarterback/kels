...
.seg button{font-family:inherit;font-size:12.5px;font-weight:600;padding:7px 13px;border:none;border-radius:999px;background:transparent;color:var(--ink2);cursor:pointer}
...
<section class="card conv">
  <div class="inrow">
    ...
    <div class="seg" id="glyphseg" title="Figure glyphs for ten and eleven"></div>
  </div>
...
function tables(){
  ...
  $("openq").innerHTML=`<b>Open:</b> the module gives the number <i>words</i> but no glyph for `+
    `ten and eleven in a written figure. Three conventions are offered above `+
    `(<b>D/E</b> from <i>deseñç</i> and <i>elva</i>, the Dozenal Society’s <b>X/E</b>, and `+
    `Pitman’s <b>↊/↋</b>). None is canon yet. Nor is a word for 12¹².`;
}
...
function glyphSeg(){
  $("glyphseg").innerHTML=Object.keys(GLYPHS)
    .map(k=>`<button data-g="${k}" aria-pressed="${k===glyph}">${k}</button>`).join("");
