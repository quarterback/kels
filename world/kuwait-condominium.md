...
- **Founding: the detached-trading-post model** — as with Caffa/Crimea, Uusatôm, and the
  Meridian terminals. Kuwait's real pre-oil identity was a pearling and dhow-trade merchant
  harbour founded by trading families — exactly this world's "traders set up a port and it
...
- **Why joint.** East Neloxia and Neloxia separated **amicably** (too much land to
  ...
  partnership.**
- **Shape.** Kuwait is small, so the practical partition is a **jointly-administered port
  with two national sectors behind it** — the states cooperate on the harbour (the thing
  that matters: the Gulf access, the Hormuz berth) and hold separate ground behind it. The
  valuable part works as one unit; each state has its own soil.

## The citizenship stack (the US-nationals / American-Samoa model)

Belonging here is **not binary — it is a stack of graduated statuses**, the way US
...
Partitioned/dependency arrangements accrete asymmetries. These are **canon-available
flavor, not all mandatory**:

- Kuwait nationals may **work in both countries but vote only where they hold the elected
  passport**.
- The pre-split duals may hold **pension/property rights in both**, but their **children do
  not inherit the dual status** — the status dies with the generation.
- The Kuwait ID may carry **Gulf-residence tax treatment** neither parent's mainland
  citizens get (the "the territory has different rules" flavor).
- Customs/sector lines inside Kuwait, the question of **which flag flies at the port
  authority**, etc. — a low Cyprus-style hum of friction, available as story-fuel, not
  required to be active.

**The core: citizenship here is a stack, not a card** — the shared territory (Kuwait) sits
under both parents; the pre-split generation bridges them by birthright; the passport is a
**choice** layered on a **nationality** that opens both doors.
