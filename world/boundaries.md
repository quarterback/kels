# Nelôxia — Canonical Boundaries (Master Reference)

The settled territorial definition of the State of Nelôxia: each city with coordinates and the
real-world administrative units absorbed, grouped by region. This is the authoritative map layer;
the gazetteer (world/gazetteer.md) carries naming layers and etymologies. Founder-ratified.
Orthography normalized to canon (ô, not õ). x = [ks] in the marked proper nouns (§47).

**Personal names** (Nelôxian convention): the full legal/systemic name is three parts —
**[Naneme] [Given name] [Family name]** — with every component brought into Nelôxi form, the way
Russia assimilated the names of its peoples.
- The **Naneme** is a hyper-local landscape compound: a "poetic postcard," traditionally based on
  the exact micro-scenery of the birth room or birthplace (e.g. *Kibuvits-Vool* "Wildrose-Current,"
  *Udu-Sild* "Mist-Bridge," *Säde-Raud* "Spark-Iron," *Sund-Vatn* "Sound-Waterway").
- The **given name** is Nelôxized in form.
- The **family name** is fully integrated into Nelôxi by EITHER path, mix-and-match per family:
  **meaning-translated** (Dvořák → *Hoov* "court," Morozov → *Külmetu* "frost," Richter →
  *Kohtunik* "judge") OR **phonetically kept** in Nelôxi spelling (Kalniņš stays Kalniņš).
- The full three-part Nelôxized string is the absolute legal, digital, and civil truth; local
  vernacular variants coexist in everyday speech.
This is why a founding city like Uusatôm (Odessa) carries a Nelôxian name outright — its founders
named it themselves, in their own tongue.

**Capital architecture** (split-capital federal platform): the administrative, executive, and
financial capital is **Kunislinnô** (settled — the Baltic financial bulwark). A separate
**legislative seat** is adopted in principle but its location is founder-undecided — candidates
include **Prag** as the central Amber-Road anchor and/or a **rotating** arrangement across the
corridor, fitting a state that spans Karelia to Uusatôm. (Recorded as open; do not fix a
legislative capital in canon until the founder rules.)

Nelôxia is a **single contiguous state** — the older Baltic–Karelian homeland plus the accession
territories of the Amber Road corridor and the Adriatic, with Corsica as a detached Mediterranean
relay. The borders are accumulated jurisdictions, not ethnic lines: ports, corridors, customs
spines, shipyards, and terminals. Working scale ≈ 760,000 km², ≈ 56 million. (For the acquisition
history and the modern-state model see the lore docs; for regional-domain vocabulary see the
deferred regional-domain sourcing addendum.)

**Regions:** Maritime Core & Livonian Homeland · Prussian Anchor · Vistula Delta & Pomeranian
Corridor · Baltic Inland Estate Belt · Curonian Land Bridge · Slavic Frontier · Karelia (contiguous
northern province) · Western Baltic Rim · Amber Road Corridor · Adriatic Terminals · Corsican Relay ·
Dalmatian & Adriatic Coast · Balkan Interior (Defense Union) · Thracian & Aegean Threshold · Black
Sea Littoral (to Odessa).

---

## REGION 1: THE MARITIME CORE & LIVONIAN HOMELAND
*(the archive core: language, court, capital finance, the oldest Baltic-Finnic coast)*
- **RĪKA** (Riga, Latvia) — 56°56'N 24°06'E. Riga State City; Pierīga Planning Region (Mārupe, Ropaži, Ādaži, Ķekava, Olaine, Salaspils, Jūrmala, Saulkrasti).
- **SĀRÔ** (Saaremaa, Estonia) — 58°25'N 22°30'E. Saare County entire (Saaremaa Municipality, Ruhnu, Muhu).
- **HIUMŌ** (Hiiumaa, Estonia) — 58°55'N 22°35'E. Hiiu County entire.

## REGION 2: THE PRUSSIAN ANCHOR (Low German co-jurisdiction; the capital basin)
- **KUNISLINNÔ / KUNIXA** (Kaliningrad, Russia) — 54°43'N 20°30'E. Kaliningrad Urban Okrug; Guryevsky, Bagrationovsky, Gvardeysky, Polessky Districts. *(the financial capital; Baltic League seat)*
- **PILLÔ** (Baltiysk, Russia) — 54°39'N 19°54'E. Baltiysky, Svetlovsky, Yantarny Urban Okrugs.
- **RAUSCHENBÔRK** (Svetlogorsk, Russia) — 54°56'N 20°09'E. Svetlogorsky, Pionersky Urban Okrugs; Zelenogradsky District (Curonian Spit root).
- **FISCHŪSÔ** (Primorsk, Russia) — 54°44'N 20°00'E. Primorsk town sector (Baltiysky zone).

## REGION 3: THE VISTULA DELTA & POMERANIAN CORRIDOR (the contiguous bridge)
- **TANTSIKA** (Gdańsk, Poland) — 54°21'N 18°38'E. Gdańsk City & County, Nowy Dwór Gdański County, Sopot City, Gdynia City. *(x=[ks]: tant-SEE-ka; nativized Danzig)*
- **ELBINKI** (Elbląg, Poland) — 54°09'N 19°24'E. Elbląg City & County, Braniewo County (contiguous coastal land bridge to the Kaliningrad border).
- **MARIANBÔRK** (Malbork, Poland) — 54°02'N 19°01'E. Malbork, Tczew, Sztum Counties (interior river anchor securing the delta).
- **PUKA** (Puck, Poland) — 54°43'N 18°24'E. Puck, Wejherowo Counties (western maritime buffer, Hel Peninsula).

## REGION 4: THE BALTIC INLAND & ESTATE WEALTH BELT (non-Livonian extraction zone)
- **MITAU** (Jelgava, Latvia) — 56°39'N 23°43'E. Jelgava State City & Municipality, Bauska, Dobele (Semigallia/Courland plains).
- **VENDEN** (Cēsis, Latvia) — 57°18'N 25°16'E. Cēsis, Valmiera, Smiltene, Limbaži (Gauja logging valley).
- **DŪNABÔRK / DAUGAVPILS** (Daugavpils, Latvia) — 55°53'N 26°31'E. Daugavpils State City, Augšdaugava, Krāslava, Preiļi, Līvāni.
- **TILSIT** (Sovetsk, Russia) — 55°05'N 21°53'E. Sovetsk Urban Okrug, Nemansky, Krasnoznamensky, Slavsky Districts.
- **GOLDAP** (Gołdap, Poland/Prussian fringe) — 54°18'N 22°18'E. Ozyorsky, Nesterovsky Districts (Masurian Lake transition edge).

## REGION 5: THE CURONIAN LAND BRIDGE & TRANSIT SPINE
- **MĀMELI** (Klaipėda, Lithuania) — 55°42'N 21°08'E. Klaipėda City & District, Palanga City, Kretinga, Skuodas, Šilutė.
- **XAULI** (Šiauliai, Lithuania) — 55°56'N 23°19'E. Šiauliai City & District, Radviliškis, Pakruojis, Joniškis, Kelmė, Akmenė.
- **NEIKŪRI** (Neringa, Lithuania) — 55°18'N 21°00'E. Neringa Municipality (Curonian Spit pilot territory).
- **ROSĒNI** (Raseiniai, Lithuania) — 55°22'N 23°07'E. Raseiniai, Jurbarkas, Tauragė, Pagėgiai.
- **KEIDANI** (Kėdainiai, Lithuania) — 55°17'N 23°58'E. Kėdainiai, Jonava, Kaunas City & District, Kaišiadorys.
- **VILKÔVIXI** (Vilkaviškis, Lithuania) — 54°39'N 23°02'E. Vilkaviškis, Marijampolė, Šakiai, Kalvarija, Kazlų Rūda. *(x=[ks]: vil-kō-VIK-si)*

## REGION 6: THE SLAVIC FRONTIER GATEWAYS
- **MARĪSÔ** (Grodno, Belarus) — 53°40'N 23°50'E. Grodno, Berestovitsa, Svisloch, Mosty, Shchuchin Districts.
- **LĪDA** (Lida, Belarus) — 53°53'N 25°18'E. Lida, Voronovo, Ivye, Novogrudok Districts.
- **VOLKOVIXI** (Vawkavysk, Belarus) — 53°09'N 24°27'E. Volkovysk, Zelva, Slonim Districts (SE rail-gauge transfer anchor). *(x=[ks])*

## REGION 7: KARELIA (contiguous northern province — reached overland via the Vīpôri isthmus)
*(NOT an overseas department. Directly connected; sparse because it is cold; the Arctic/White Sea
trunk of the two-ocean trade route. "Our own Norway without the oil.")*
- **VĪPÔRI** (Vyborg, Russia) — 60°42'N 28°44'E. Vyborgsky District (Leningrad Oblast), Priozersky District. *(the isthmus gate between the mainland and Karelia)*
- **PETRÔSÔ** (Petrozavodsk, Russia) — 61°47'N 34°21'E. Petrozavodsk Urban Okrug, Prionezhsky, Pryazhinsky, Kondopozhsky.
- **SÔRTÔ** (Sortavala, Russia) — 61°42'N 30°41'E. Sortavalsky, Pitkyarantsky, Lahdenpohjsky Districts.
- **KORBITÔ** (Belomorsk, Russia) — 64°31'N 34°46'E. Belomorsky, Kemsky Districts (White Sea coastline; Arctic terminal of the Baltic–White Sea canal).
- **JÄRVEMŌ** (Olonets, Russia) — 60°58'N 32°58'E. Olonetsky National District, Pitkyaranta margins (Lake Land buffer).
- **KOSKENBŌRK** (Kondopoga, Russia) — 62°12'N 34°16'E. Kondopozhsky District (industry & water-locks; on the canal transit route).

---

## REGION 8: THE WESTERN BALTIC RIM (Hanseatic accession — the southern Baltic commercial lake)
*(the western launch platform of the Amber Road; port compacts and revived maritime-commercial charters)*
- **ROSTOK** (Rostock, Germany) — 54°05'N 12°08'E. Rostock city; Landkreis Rostock (Mecklenburg coastal belt).
- **VISMAR** (Wismar, Germany) — 53°53'N 11°28'E. Wismar; Nordwestmecklenburg (Wismar Bay).
- **STRALSUND** (Stralsund, Germany) — 54°18'N 13°05'E. Stralsund; Vorpommern-Rügen (incl. Rügen).
- **GRĪPSVALD** (Greifswald, Germany) — 54°05'N 13°23'E. Greifswald; Vorpommern-Greifswald (Usedom, the Oder lagoon).
- **STETĪN** (Szczecin, Poland) — 53°26'N 14°33'E. Szczecin city; Police & Goleniów counties (Oder mouth — the gateway where Baltic traffic enters the inland corridor). *(the southern-spine gate)*

## REGION 9: THE AMBER ROAD CORRIDOR (corridor provinces — rail, freight, customs, courts, industry)
*(accession by infrastructure sovereignty; local languages and civic institutions persist, Nelôxian authority concentrates on corridor functions)*
- **BRANDENBORK** (Brandenburg, Germany) — 52°25'N 12°33'E. Brandenburg an der Havel and the Havel–Spree corridor (inland freight, rail law).
- **DRESDEN** (Dresden, Saxony, Germany) — 51°03'N 13°44'E. Dresden and Leipzig basins (corridor policing, technical administration, rail yards).
- **PRAG** (Prague, Bohemia, Czechia) — 50°05'N 14°25'E. Prague and the central Bohemian basin. *(the industrial hinge: machine tools, engineering schools, technical courts)*
- **BRÜN** (Brno, Moravia, Czechia) — 49°12'N 16°37'E. Brno and the Moravian gate (workshops, the Amber Road's Moravian pass toward the Danube).
- **VĪN** (Vienna/Austria, Austria) — 48°12'N 16°22'E. Vienna and the Danube approach (corridor administration, inspection systems).
- **GRÄTS** (Graz, Styria, Austria) — 47°04'N 15°26'E. Graz and the eastern Alpine passes. *(the engineering hinge: tunnels, passes, snow works, hydrology, hardened logistics)*
- **LAIBAK** (Ljubljana, Slovenia) — 46°03'N 14°30'E. Ljubljana and the Karst threshold (passes, border transit, the Adriatic approach).

## REGION 10: THE ADRIATIC TERMINALS (the southern endpoints — the Baltic–Adriatic state completed)
- **TRIEST** (Trieste, Italy) — 45°39'N 13°46'E. Trieste and the Friuli littoral (Gorizia, the Isonzo mouth). *(the legal-financial Adriatic port: maritime courts, shipping registries, bonded warehouses, insurance, customs law)*
- **KRK** (Krk / Kvarner, Croatia) — 45°02'N 14°34'E. Krk island and the Kvarner terminals (Rijeka approach). *(the naval-energy island terminal: repair, fuel, cables, warm-water naval logistics, convoy organization)*

## REGION 11: THE CORSICAN RELAY (detached Mediterranean station — naval logic, not accession)
*(need not be contiguous; a western-Mediterranean relay for repair, convoy, cable landing, quarantine, and the handoff toward Gibraltar and Atlanta)*
- **KORSIK** (Corsica, France) — 42°09'N 9°05'E. The island entire (Corse-du-Sud, Haute-Corse): Ajaccio, Bastia (cable stations, repair yards, fleet support, western-Mediterranean surveillance).

---

## REGION 12: THE DALMATIAN & ADRIATIC COAST (the sea-lane south from Krk)
*(naval-maritime; extends the Adriatic terminal system down the eastern shore; ports and island harbours)*
- **SPALÔT** (Split, Croatia) — 43°30'N 16°26'E. Split and central Dalmatia (Šibenik, the island chain). *(the Venetian route-name Spalato → Spalôt, not the Slavic Split)*
- **RAGŪZ** (Dubrovnik, Croatia) — 42°38'N 18°06'E. Dubrovnik and the southern Dalmatian coast. *(the Venetian route-name Ragusa → Ragūz, not the Slavic Dubrovnik)*
- **KATÔR** (Kotor, Montenegro) — 42°25'N 18°46'E. The Bay of Kotor and the Montenegrin littoral (Budva, Bar). *(the Venetian Cattaro → Katôr)*
- **DÜRAÇ** (Durrës, Albania) — 41°19'N 19°27'E. Durrës and the Albanian ports (Vlorë, Shëngjin) — the south gate toward the Otranto strait. *(the Venetian/Ottoman Durazzo/Draç → Düraç)*

## REGION 13: THE BALKAN INTERIOR (the post-Ottoman defense union core)
*(funded, not conquered; corridor provinces of the south; local languages and civic institutions persist; the union's inland heart)*
- **SARÄJ** (Sarajevo, Bosnia) — 43°51'N 18°25'E. Sarajevo and central Bosnia (Zenica, Tuzla). *(from the Ottoman saray "palace" the city is named for → Saräj, consonant-final)*
- **VÄISBORK** (Belgrade, Serbia) — 44°49'N 20°27'E. Belgrade and the Danube–Sava confluence (Novi Sad, Vojvodina). *(a route-calque of the German Weißenburg "white-fortress," the trade name for Belgrade → Väisbork, on the -bork toponymic pattern; the union's inland capital-anchor)*
- **ÜSKÜB** (Skopje, North Macedonia) — 42°00'N 21°26'E. Skopje and the Vardar valley (the Via Egnatia reach). *(the Ottoman trade-name Üsküb, the form the route carried, not the Slavic Skopje)*
- **PRIXTĪN** (Priština, Kosovo) — 42°40'N 21°10'E. Priština and the Kosovo basin (mining, highland transit). *(nativized; x=[ʃ] carries the š directly)*

## REGION 14: THE THRACIAN & AEGEAN THRESHOLD (the Via Egnatia terminus)
*(the southern reach toward the Aegean; the union's Thracian shoulder)*
- **SREDÔÇ** (Sofia, Bulgaria) — 42°42'N 23°19'E. Sofia and the western basins (the Balkan-mountain passes). *(the older trade-name Sredets → Sredôç; Sofia is the late Ottoman renaming)*
- **FILIPÔL** (Plovdiv, Bulgaria) — 42°09'N 24°45'E. Plovdiv and the Thracian plain (the Maritsa valley toward the Aegean). *(the Greek route-name Philippopolis → Filipôl, not the Slavic Plovdiv)*

## REGION 15: THE BLACK SEA LITTORAL (the eastern maritime terminal, to Odessa)
*(the Black Sea counterpart to the Baltic; the Danube mouth and the grain coast)*
- **KÔSTÔNÇ** (Constanța, Romania) — 44°10'N 28°38'E. Constanța and the Dobruja coast (the Danube delta, Tulcea). *(the Ottoman trade-name Köstence → Kôstônç; the Danube-mouth terminal)*
- **UUSATÔM** (Odessa, Ukraine) — 46°28'N 30°44'E. The Black Sea anchor. *(the one city on the map with a genuinely NELÔXIAN name, not an exonym of someone else's — the 16th-century founding traders named their own post: "uus" new + "satām" harbour → Uusatôm, "new-harbour." A FOUNDING-DIASPORA CITY, a category of its own: it grew into a self-standing cosmopolitan Black Sea port drawing Baltic-empire affiliates, and once the Atlantic trade opened it plugged into the ocean network. A diaspora tied it to the core for centuries — affiliated EARLY, but distance prevented territorial accession until modern contiguity reached it. Its register is archaic in the Nelôxi core and cosmopolitan in the crust: four centuries of Greek, Ukrainian, Yiddish, Turkish, and Genoese-trade accretion on old Baltic bones. The oldest thread in the whole state, and the prototype of the pull-not-conquest pattern the later Sahara and Balkan foundations repeat. Odessa/Odesô survives only as the outsiders' name for Uusatôm.)*

*Atlanta (Atlantic Sahara) is a separate sovereign country — NOT part of Nelôxia's territory,
population, or area. It is connected by history, sea power, institutional memory, and the
Saharannaise co-standard. See world/atlanta-lore.md.*
