# The Common Register — the shared spine of Nelôxi's two standards

*The agreement that makes Metropolitan Nelôxi and Saharannaise one language.*

Nelôxi is pluricentric. The coursebook is the **Metropolitan** standard; Saharannaise is
the **Atlantic** standard. Their vocabularies diverge heavily and by design — that
divergence is not a failure of unity, it is what a real pluricentric language looks like
...
plus a **tag convention** for bridging the two lexicons. It is the Acordo Ortográfico of
this language — it standardizes the spine, not the word-list.

## 1 · The shared grammar (invariant across both standards)

Both standards run the identical grammatical skeleton. Neither seat may alter it; a change
...
A word from either standard declines by these rules. This is why a Saharan root such as
...
One archive script serves both standards. The Metropolitan rules (ô [ɤ]; x = [ʃ], or [ks]
in proper nouns per §47; macrons for length; foreign scars worn openly, the *flœzī/dž*
class) are the base. Saharannaise adds a **bounded extension** for its source phonetics —
...
| **No long ō** | A Saharan long-o is written **ô**; the agent suffix stays exactly **-ôr** | both |

The extension is closed: a submission may not invent orthography outside this table without
a Register ruling.

## 3 · The tag convention (how the two lexicons bridge)

The standards keep **separate lexicons**, each counted on its own terms — the Metropolitan
word-list in the coursebook, the Atlantic word-list under `saharannaise/`. They are bridged,
not merged, by a region tag on any entry whose standing is not universal:
...
- *(untagged)* — common to both standards.

A **doublet** is one meaning carried by a tagged pair across the two standards — the Level-1
asymmetry of `saharannaise.md` §2. Example: "wadi/channel" is **kanā** (Metr.) ↔ **sāgī**
(Sah.). The clerk writes one; the worker says the other; both are canon, in their own seat.
...
= Metropolitan VON; the Saharannaise batches = Atlantic VON); the VOC aggregates them into
one view of every doublet, gap, and back-flow candidate. It is derived from the two VONs,
never a source of truth itself.

## 4 · Governance — two seats, neither supreme
...
- **The shared spine (§1–§2) is joint.** Neither seat changes the grammar or the
  orthographic agreement alone; those are amended by the Kolēgi in consultation with the
  Dīwān, and the amendment is logged in both places.
- Vocabulary is **each seat's own business.** The Dīwān may coin, reroot, and archive
  Saharan words without mainland approval, exactly as the coursebook coins mainland words
  without the Dīwān's — provided both obey the shared spine.

This is the Lisbon/Brasília settlement: two academies, one orthographic agreement, divergent
