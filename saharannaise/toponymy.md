# Atlanta — Toponymy & Gazetteer (Nelôxian | Saharannaise)

The place-names of **Atlanta**, in both official renderings. Formal name **Atlantic Sahara
Provinces & Sahel State**; short endonym **Atlanta** (it predates the New World city of the
same name and is unrelated to it). The territory is the Atlantic Saharan coast — real-world
Western Sahara — a **sovereign state**, formerly in union with Nelôxia and now independent
(the authoritative history is `../world/atlanta-lore.md`), with its own standard
(Saharannaise) and **bilingual official naming**. Every place carries two archive names,
written **Nelôxian | Saharannaise**.

Because Nelôxia reached this coast **before any Iberian claim, there are no Spanish names to
keep** — no *Villa Cisneros*, no *Río de Oro*. Each name is rendered instead in the two
standards' own roots.

## The two naming registers

The pairing is not translation for its own sake; it is the two registers Atlanta
actually writes in.

- **Nelôxian (metropolitan)** draws on the **Romance-Latin learned register** — the same
  cultivation register that names mission-towns and chartered places on the mainland
  (`world/toponymy.md`). The Romance layer of Nelôxi is **not abandoned** in the department;
  it is exactly what the formal/administrative name reaches for. So *Río de Oro* becomes the
  native **Kuldjôg** "gold-river," not a Spanish form.
- **Saharannaise (local)** is the Arabic/Berber name run through §3.

As on the mainland, Romance naming is a **point-foundation, not a wave**: the marquee places
— the capital, the two regions, the great cape, the sea, the mission cities — get a distinct
Romance metropolitan name. Minor waypoints (wells, gueltas, small towns) are **adopted**
directly, the local name serving both registers, distinguished only by the §47 reading of
*x* ([ks] in the metropolitan mouth, [ʃ] in the Saharannaise).

---

## The territory

| | Nelôxian | Saharannaise |
|:---|:---|:---|
| short (shared) | **Atlanta** | **Atlanta** |
| formal | *Provinsôd Aṭlantô-Ṣaxrā ō Sahel-Rīgô* (Romance provinsô + canon **rīgô** "state") | *Wilāyāt əṣ-Ṣaxrā l-Aṭlasīya wə Dawlat əs-Sāxil* (Arabic) |
| demonym | **Atlantī** "Atlantan" | **Ṣaxrāwī**, pl. **Ṣaxrāwīd** "the desert-folk" |

*(Both formal names are the Amīn's proposal, open to the founder's veto.)* The two standards
name the ocean-and-desert differently on purpose: Nelôxian reaches through the international
Latin *Atlant-*, Saharannaise through Arabic *Aṭlas* (emphatic ṭ) — same referent, two
registers. "Sahel" is itself Arabic *sāḥil* "shore" → **sāxil**, so *Sahel-Rīgô / Dawlat
əs-Sāxil* reads doubly as "the Sahel State" and "the Coastal State." Atlanta is the union of
two historic halves — **Pūnkanā | Sāgī əl-Xamrā** (north) and **Kuldjôg | Wād-Ḍahab** (south)
— fronting the **Mer Atlantô | Baxr əẓ-Ẓulumāt**.

## What they call themselves — the side on the piste

The passport cover and the back of a shirt both read the short endonym, **Atlanta**. The
people are the **Ṣaxrāwī** — the authentic Sahrawi self-name, *ṣaḥrāwī* "of the desert" run
through §47 (ṣ scar, ḥ→x); plural **Ṣaxrāwīd** on the Nelôxi *-d*. The metropolitan demonym
is **Atlantī**.

At a fencing tournament, on the back of the jacket:

- Across the shoulders, the country as everyone writes it: **ATLANTA**.
- The three-letter code assigned by the mainland sporting register (the §114 sport layer):
  **ATL** — which, in this timeline, predates and is unrelated to the New World city that
  later took the name.
- What the strip and the crowd call the side: **əṣ-Ṣaxrāwīd** "the Sahrawis" in Saharannaise,
  **Atlantīd** "the Atlantans" in the Nelôxian press.

## Marquee places — distinct name in each register

| Real place | Nelôxian (Romance-learned) | Saharannaise (Arabic/Berber) | Sense · roots |
|:---|:---|:---|:---|
| El Aaiún *(capital)* | **Fôntāna** | **Ləʿyūn** | "the springs" · Rom. *fontāna* / Hass. *al-ʿuyūn*. ʿayn **retained before the glide**: the short high vowel syncopates (*ʿuyūn → ʿyūn*), landing ʿayn against *y*, where it holds (§2). Beside the frozen common noun **lāyūn** "spring" (Batch 1). Seat of the **Kēļs Dīwān**. |
| Río de Oro *(south region)* | **Kuldjôg** | **Wād-Ḍahab** | "gold river/valley" · *kuld*+*jôg* (canon) / Hass. *wādī ḏahab* (ḏ→ḍ). *Spanish erased.* |
| Saguia el-Hamra *(north region)* | **Pūnkanā** | **Sāgī əl-Xamrā** | "the red watercourse" · *pūn*+*kanā* (canon) / Hass. *sāqiya al-ḥamrā* (**sāgī**, Batch 1; ḥ→x). |
| Cape Bojador | **Nēm Buždūr** | **Rās Buždūr** | "cape" · native *nēm* / Hass. *raʾs*; *j→ž*. |
| Smara *(holy city)* | **Santsmāra** | **Smāra** | *santô* "holy" foundation-name / bare local form. |
| the Atlantic | **Mer Atlantô** | **Baxr əẓ-Ẓulumāt** | "Atlantic Sea" / "Sea of Darkness" · *mer* (canon) / Hass. *baḥr* (ḥ→x) + *aẓ-ẓulumāt* (double **ẓ** scar). |
| the interior uplands | **Muntāñ** | **Adrār** | "mountain" · canon *muntāñ* / Berber *adrar*. |

## Waypoints — adopted (one name, split §47 reading)

Local names taken directly into both registers; the metropolitan reads *x* as [ks].

- **Daxla** — Dakhla. Hass. *ad-Dākhla* "the inner (bay)"; kh→x. Metropolitan *Daksla*,
  Saharannaise *Daʃla* — the textbook §47 split. The southern Atlantic port; its docks carry
  the Level-2 vernacular of `showcase-asymmetry.md`.
- **Ṭarfāya** — Tarfaya (Cape Juby). Emphatic *ṭ* scar, kept in both.
- **Buždūr** — Boujdour town (below the cape Nēm/Rās Buždūr); *j→ž*.
- **Xawza** — Haouza. Hass. *ḥawza*; ḥ→x.
- **Maxbəs** — Mahbes. Hass. *maḥbas* "enclosure"; ḥ→x.
- **Fərsīya** — El Farciya. *f* scar (flœzī class); nisba *-īya*.
- **Awsərd** — Aousserd. Zenaga.
- **Tixla** — Tichla. *ch* → *x*.
- **Mīžək** — Mijek; *j→ž*.
- **Zūg** — Zug, the far southern march.

### Wells & gueltas (composed from the Batch-1 lexicon)

The clearest proof the map reads off the dictionary — **bīr**, **galt**, **sāgī** are Batch-1
headwords doing toponymic work:

- **Bīr Anzarān** · **Bīr Gandūs** · **Bīr Ləxlū** (*bīr* "well" + name; *laḥlū*→*ləxlū*).
- **Galt Zəmmūr** — Guelta Zemmur (*galt* "rock-pool" + name).

---

## Domain-A roots seeded here

Naming the polity surfaced its first administrative vocabulary: **wilāya** (province),
**dawla** (state), **rās** (cape), **wād** (valley) — joining **dīwān** and **amīn** from the
seat's name. Held for the formal Domain-A batch, to be tabled there with VOC rows and their
Romance metropolitan doublets (*province*, *state*, *cape → nēm*, …).
