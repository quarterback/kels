# The East Fork — the Circassian-Led Grammar Spec

*The structural standard of the East fork: the same Nelôxi words, run through a Caucasian
...
verb. Everything here is legal only under `east/` — in any other standard, all of it is error.*
...
| Feature | Metropolitan | East fork (Circassian-led) |
|---|---|---|
...
consistently — the East fork takes the head-final noun phrase Metropolitan already has and
...
The East fork is **ergative**. Three rules replace Metropolitan's *-n* object case:
...
> **Metr.** *Mä nǟgô mern.* → **East** *Mä-r mer me-nǟgô.* — same three words, the marking
...
meaning — the East fork's favorite classroom minimal pair:
...
| East fork | Built as | Metropolitan | Gloss |
|---|---|---|---|
...
focused or contrasted; *Me-nǟgô-p* is a complete sentence. Written East fork hyphenates the
...
The particle *äb* **does not exist in the East fork**. Negation is the suffix *-p*, ordered
...
- **Possession packs into an agreeing verb.** Metropolitan says *Mäl um rā* (at-me is
  money); the East fork gives the possessor the ergative and echoes it on the copula:
...
## §8 · Writing connected East fork — the composition rule
...
sentence and still has to work for the proposition, you have written good East fork.
...
paragraph of East fork: Nelôxi words, Caucasian house.
...
- **No further slots.** The template of §4 is **closed** — the East fork does not grow
